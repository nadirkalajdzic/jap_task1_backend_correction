...
namespace JapTask1BackendCorrection.Infrastructure.Services.MediaService
{
    public class MediaService : IMediaService
    {
        ...
        public async Task<ServiceResponse<List<GetMediaDTO>>> GetMedias(GetMediasRequest request)
        {
            if(request == null)
            {
                request = new();
            ...
            return new()
            {
                Data = await _context.Medias
                                     .Include(x => x.Ratings)
                                     .Where(x => x.Type == request.MediaType)
